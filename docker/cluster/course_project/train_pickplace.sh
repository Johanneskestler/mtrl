#!/bin/bash
# SLURM job script for Single-Task SAC training: PICK-PLACE
# Course Project: From Single-Task to Multi-Task RL
# Expected success rate: > 30%

#SBATCH --job-name=ST_SAC_pickplace
#SBATCH --partition=GPU-l40s
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=04:00:00
#SBATCH --output=/home/e11704784/metaworld_project/logs/pickplace_%j.log
#SBATCH --error=/home/e11704784/metaworld_project/logs/pickplace_%j.err
...
RESULTS_DIR="${PROJECT_DIR}/results/pickplace_${SLURM_JOB_ID}"
SEED="${SEED:-1}"
...
echo "=========================================="
echo "Single-Task SAC Training: PICK-PLACE"
...
singularity exec --nv \
    ...
    --pwd /source \
    "${CONTAINER}" \
    python experiments/course_project/mt1_pick_place.py \
        --seed "${SEED}" \
        --track \
        --wandb-project "Robot_learning_2025" \
        --wandb-entity "Robot_learning_2025" \
        --data-dir "/workspace/results/pickplace_${SLURM_JOB_ID}"

EXIT_CODE=$?
echo ""
echo "=========================================="
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ PICK-PLACE training completed!"
else
    echo "❌ PICK-PLACE training failed!"
fi
echo "=========================================="
exit $EXIT_CODE
